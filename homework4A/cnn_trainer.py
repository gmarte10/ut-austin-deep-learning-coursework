...
def cnn_trainer(
        ...
        exp_dir: str = "cnn_logs", # Defualt dir for testing
    ...
    val_data=load_data("drive_data/val", shuffle=False, num_workers=2)
    ...
    for epoch in range(num_epoch):
        ...
        if (val_lat_e < 0.3 and val_long_e < 0.2) or (early_stop_counter >= patience):
            save_model(model)
            torch.save(model.state_dict(), log_dir / f"{model_name}.th")
            print(
    ...
    print(f"Model saved to {log_dir / f'{model_name}.th'} | Saved at epoch {saved_epoch}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--exp_dir", type=str, default="mlp_logs")
    ...
    cnn_trainer(**vars(parser.parse_args()))
